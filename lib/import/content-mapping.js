...
export function rowToContent(row, headers, clientId) {
    ...
    // Safety: if the only "title" keyword found is actually the "blog" column (which is usually a link),
    // and we have a proper blog doc mapping elsewhere, or if it's the exact same column,
    // we should NOT use the 'blog' column as the title.
    if (titleField && titleField === blogDocField && normHeader(titleField) === 'blog') {
        // Fallback to the first header that isn't the blog link
