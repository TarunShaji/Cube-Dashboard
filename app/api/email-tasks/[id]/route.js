...
import { EmailTaskUpdateSchema } from '@/lib/db/schemas/email.schema'
...
export async function GET(request, { params }) {
    return withAuth(request, async () => {
        ...
        const task = await database.collection('email_tasks').findOne({ id: taskId })
        if (!task) return handleCORS(NextResponse.json({ error: 'Email task not found' }, { status: 404 }))
...
export async function PUT(request, { params }) {
    return withAuth(request, async () => {
        ...
        const validation = validateBody(EmailTaskUpdateSchema, body)
        ...
        const current = await database.collection('email_tasks').findOne({ id: taskId })
        if (!current) return handleCORS(NextResponse.json({ error: 'Email task not found' }, { status: 404 }))
        ...
        const result = await database.collection('email_tasks').updateOne(
        ...
        const updated = await database.collection('email_tasks').findOne({ id: taskId })
...
export async function DELETE(request, { params }) {
    return withAuth(request, async () => {
        const { id: taskId } = params
        const database = await connectToMongo()
        const result = await database.collection('email_tasks').deleteOne({ id: taskId })
        if (result.deletedCount === 0) {
            return handleCORS(NextResponse.json({ error: 'Email task not found' }, { status: 404 }))
        }
        return handleCORS(NextResponse.json({ message: 'Email task deleted' }))
    })
}
