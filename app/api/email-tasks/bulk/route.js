...
export async function POST(request) {
    return withAuth(request, async () => {
        ...
        const validation = validateBody(EmailBulkSchema, body)
        ...
        if (finalTasks.length > 0) {
            await database.collection('email_tasks').insertMany(finalTasks)
        }

        return handleCORS(NextResponse.json({
            count: finalTasks.length,
            message: `Successfully imported ${finalTasks.length} email tasks`
        }))
    })
}
...
export async function DELETE(request) {
    return withAuth(request, async () => {
        ...
        const result = await database.collection('email_tasks').deleteMany({ id: { $in: ids } })

        return handleCORS(NextResponse.json({
            message: `Successfully deleted ${result.deletedCount} email tasks`,
