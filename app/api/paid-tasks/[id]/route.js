...
import { PaidTaskUpdateSchema } from '@/lib/db/schemas/paid.schema'
...
export async function GET(request, { params }) {
    return withAuth(request, async () => {
        ...
        const task = await database.collection('paid_tasks').findOne({ id: taskId })
        if (!task) return handleCORS(NextResponse.json({ error: 'Paid task not found' }, { status: 404 }))
...
export async function PUT(request, { params }) {
    return withAuth(request, async () => {
        ...
        const validation = validateBody(PaidTaskUpdateSchema, body)
        if (!validation.success) {
            return handleCORS(NextResponse.json(validation.error, { status: 400 }))
        }

        const cleanUpdate = validation.data
        const current = await database.collection('paid_tasks').findOne({ id: taskId })
        if (!current) return handleCORS(NextResponse.json({ error: 'Paid task not found' }, { status: 404 }))
        ...
        const updated = await database.collection('paid_tasks').findOne({ id: taskId })
...
export async function DELETE(request, { params }) {
    return withAuth(request, async () => {
        const { id: taskId } = params
        const database = await connectToMongo()
        const result = await database.collection('paid_tasks').deleteOne({ id: taskId })
        if (result.deletedCount === 0) {
            return handleCORS(NextResponse.json({ error: 'Paid task not found' }, { status: 404 }))
        }
        return handleCORS(NextResponse.json({ message: 'Paid task deleted' }))
    })
}
