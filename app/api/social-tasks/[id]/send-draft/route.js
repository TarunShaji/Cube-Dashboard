...
/**
 * POST /api/social-tasks/[id]/send-draft
 * Marks content_draft_sent = true, resets content_draft_approval to Pending.
...
export async function POST(request, { params }) {
    return withAuth(request, async () => {
        try {
            ...
            const finalUpdate = applySocialTaskTransition(current, { content_draft_sent: true })
            ...
            return handleCORS(NextResponse.json({ message: 'Content draft sent to client portal', task: responseBody }))
        } catch (error) {
